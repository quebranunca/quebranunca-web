...
export function CriarGrupoFluxo({
  modoExibicao = 'modal',
  onFechar,
  onCriado,
  fecharAoCriar = true
  ...
  const [grupoCriadoComFalhaUpload, setGrupoCriadoComFalhaUpload] = useState(null);
  ...
  function fecharSemConfirmacao() {
    ...
    setGrupoCriadoComFalhaUpload(null);
  ...
  async function criarGrupo() {
    ...
    if (grupoCriadoComFalhaUpload) {
      await onCriado?.(grupoCriadoComFalhaUpload);
      if (fecharAoCriar) {
        fecharSemConfirmacao();
      }
      return;
    ...
    try {
      ...
      if (arquivoImagemGrupo) {
        try {
          ...
          setGrupoCriadoComFalhaUpload(grupo);
  ...
  const tituloFluxo = etapa === 3 ? 'Confirmar grupo' : 'Criar grupo';
  const ehModal = modoExibicao === 'modal';

  return (
      <div
        ref={modalRef}
        className={`criar-grupo-modal criar-grupo-fluxo criar-grupo-fluxo-${ehModal ? 'modal' : 'pagina'}`}
        {...(ehModal ? {
          role: 'dialog',
          'aria-modal': 'true',
      ...
      >
        ...
        {modalConfirmacaoSaidaAberto && (
          <div className="criar-grupo-confirmacao-backdrop" role="presentation">
            <section className="criar-grupo-confirmacao" role="dialog" aria-modal="true" aria-label="Deseja sair da criação do grupo?">
        ...
        {modalSimilaresAberto && (
          <div className="criar-grupo-similares-backdrop" role="presentation">
            <section className="criar-grupo-similares" role="dialog" aria-modal="true" aria-label="Encontramos grupos parecidos">
